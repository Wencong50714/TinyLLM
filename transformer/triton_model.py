...
from transformer.flash_attention import TritonAttentionFunction
from transformer.utils import *
from transformer.rms_norm import TritonRMSNorm
from transformer.softmax import fused_softmax
...
class TritonAttention(torch.nn.Module):
    ...
    def forward(
        ...
        output = TritonAttentionFunction.apply(xq, xk, xv, True, softmax_scale).half()
...
class DecoderLayer(nn.Module):
    def __init__(self, layer_id: int, args: ModelArgs):
        ...
        self.attention = TritonAttention(args).half()
    ...
    def forward(self, x, freqs_cos, freqs_sin):
        ...
        h = x + self.attention.forward(
            self.attention_norm(x).half(), freqs_cos, freqs_sin
        )
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        # 通过词嵌入层和Dropout层
        h = self.tok_embeddings(tokens).half()
        h = self.dropout(h)
        # 获取相对位置嵌入的频率
...
if __name__ == "__main__":
    ...
    x = torch.randint(0, 32000, (2, 64)).cuda()  # [bs, seq_len]
    # 实例化LLaMA2Model
